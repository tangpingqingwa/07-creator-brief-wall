...
Public English site: weekly board of briefs worth taking. One form (brand, terms, brief URL, amount, Outbid). Waffo takes USD. Cards show **$** and **clicks**. Live rank is the rolling last 7 days from paid placement, not Monday 00:00 UTC.
...
| Payments | Explicit `fixture`, `waffo-test`, or `waffo-prod` mode. `FixturePaymentPort` in tests; Waffo uses the official SDK with mode-scoped config. |
| CSS | Minimal utility CSS or Tailwind. Visual clone: one input row, amount, Outbid button, ranked cards with $ and clicks. |
| Tests | `node:test` + fixture Waffo. No live Waffo in `scripts/test.sh`. |
...
src/
  ...
  lib/
    ...
    urls.ts
    week.ts
    polar.ts                   # Waffo adapter + fixture compatibility module
    clicks.ts
  db/schema.sql
tests/
  rank.test.ts
...
Week key: ISO week in UTC (`weekId`) is a Waffo/audit label. Live rank is a rolling last-7-days filter on paid `createdAt`, not Monday 00:00 UTC. Reset is that query filter, not a delete, plus a documented operator/test clock. Raise identity is the same canonical brief URL still inside that window — not `weekId`.
...
| `checkout.test.ts` | FixturePaymentPort; unpaid session does not list; webhook/fixture completion lists; same brief still in last 7 days raises after `weekId` rolls |
| `polar-webhook.test.ts` | Waffo signed checkout, intent recovery, exact replay, tax, stale raise, restart, and return-state vectors |
| `waffo-route.test.ts` | Canonical raw-body Waffo route, invalid signature, retryable 5xx, and exact retry |
| `provider-selection.test.ts` | Explicit mode, scoped secrets, public endpoint and durable DB fail-closed checks |
| `schema.test.ts` | Schema and exception-safe migration rollback/FK restoration |
| `scripts/test.sh` | contract files + (once app exists) `tsc` + `node:test`. Never Waffo live. |
...
- **Description:** Next.js/TS skeleton, SQLite schema (listings + payments), `GET /healthz`, extend `scripts/test.sh`. No Waffo live. No board cards yet beyond an empty honest page.
...
- **Description:** Outbid-like board: brand, terms, brief URL, amount, Outbid button, ranked cards with **$** and **clicks**. Still fixture-only money (in-memory or direct DB insert behind a test hook is OK if labeled test-only). No live Waffo.
...
- **Description:** `POST /checkout` starts Waffo or `FixturePaymentPort`. Webhook / fixture completion writes the listing. Unpaid sessions do not list. CI stays on the fixture.
- **Files:** `src/lib/polar.ts` (Waffo adapter), `src/app/api/checkout/route.ts`, `src/app/api/webhooks/waffo/route.ts`, `src/app/api/webhooks/polar/route.ts` (410 compatibility), `src/app/checkout/return/page.tsx`, `tests/checkout.test.ts`, `.env.example`
- **Dependencies:** PR 2
...
- **Description:** ISO week in UTC as Waffo/audit label. Live board is rolling last 7 days from paid placement, not Monday 00:00 UTC. `GET /r/:id` confirms terms + URL. `POST /r/:id` increments public clicks and 302s to the canonical brief URL with no added trackers.
...
- **Description:** Operator `scripts/live-smoke.sh` + `docs/live-smoke.md`. Walks SPEC §11 against a local process in explicit fixture mode. Optional Waffo test/prod configuration is fail-closed and never inferred; missing mode-scoped secrets are recorded as `BLOCKED-SECRET`. **Not** called from `scripts/test.sh` or `.github/workflows/ci.yml`.
...
- **Acceptance:** Script executable. CI runs only `WAFFO_MODE=fixture`, with no provider calls. Each SPEC §11 flow is PASS, PASS-ERROR, or BLOCKED-SECRET.
...
GA / fleet **done** also requires an operator run of `scripts/live-smoke.sh` against a local process with an explicitly configured Waffo mode **or** documented `BLOCKED-SECRET`, results in `docs/live-smoke.md`. Fixture-only CI is not done.
