...
Empty week: honest empty state. Do not seed fake briefs. Empty plaster already leads with Claim #1, so it has no Post a brief hop, no first-write Post after Open, and no first-read Open after Post.
...
On the occupied wall, a first-time creator reads **Terms** as the labeled prize on the taped flyer. `$bid` and clicks stay later facts. They do not have to open the confirm sheet to know the payout summary. After Terms, **Open brief** is the certain next hop — the first click. After Post is concentrated as the first write, that same Open brief hop is the first read. Leave still confirms on GET `/r/:id`.

On the occupied wall, a first-time buyer who came to post hops **Post a brief** after Open brief — after the flyers, not before them — landing on Claim #1. After Open wins the first click, that same Post a brief hop is the first write. Empty plaster still leads with Claim #1 and has no Post a brief hop.
